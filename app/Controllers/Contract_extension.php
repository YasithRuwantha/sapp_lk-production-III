...
use App\Models\ContractExtentionModel;
use App\Models\ContractModel;
use App\Models\FileregisteryModel;
use App\Models\LinkContractExtentionFileModel;

class Contract_extension extends BaseController
{
    private $data;
    ...
    public function __construct()
    {
        ...
        $this->data['entity_model'] = new ContractExtentionModel();
        $this->data['entity_model_1'] = new ContractModel();  
        
        $this->data['reg_model'] = new FileregisteryModel();
        $this->data['link_model'] = new LinkContractExtentionFileModel();
	...
	{
        auth_rd();
        $this->data['active_module'] = "/contract_extension/list_all/";
        $this->data['csrf'] = 1;
        $this->data['entity_id'] = $entity_id;
        
        $this->data['list_all'] = $this->data['entity_model']->select("contract.contract_name, contract.contract_number, contract_extention.*")
                            ->join('contract', 'contract.id = contract_extention.contract_id', 'left')                            
                            ->where($this->get_filter())
                            ->findAll();

        return view('contract_extension/list_all',$this->data);
    }

    public function add_edit($entity_id=0,$id=0)
	{
        auth_rd();
        $this->data['active_module'] = "/contract_extension/add_edit/";
        $this->data['csrf'] = 1;
        $this->data['entity_id'] = $entity_id;
        ...
        $this->data['list_docs'] = $this->data['link_model']->select("file_registry.*")
                            ->join('file_registry', 'file_registry.id = link_contract_extention_file.file_id', 'left')
                            ->where("link_contract_extention_file.contract_extention_id",$id)
                            ->findAll();

        return view('contract_extension/add_edit',$this->data);
    }
    ...
    private function process_form_add_edit($entity_id=0,$id=0)
    {
        ...
        if(isset($_POST['csrf']))
        {
            ...
            $this->data['details'] = [
                'extention_approved' => $this->request->getVar('extention_approved'),
                'extention_date' => $this->request->getVar('extention_date'),
            ...
            { 
                ...
                for( $i=0 ; $i < $total ; $i++ ) {
                    if(is_file($_FILES["img"]["tmp_name"][$i])) 
                    {
                        ...
                        if(move_uploaded_file($_FILES["img"]["tmp_name"][$i], $target_file)) 
                        {
                            ...
                            $this->data['file_link'] = [
                                'contract_extention_id' => $this->data['id'],
                                'file_id' => $file_id,
                            ];
                ...
                header("Location:" . base_url("/contract_extension/list_all/" . $entity_id . "/" . $id . "/")); 
                die;
    ...
    public function delete($entity_id=0,$id=0)
    {
        ...
        header("Location:" . base_url("/contract_extension/list_all/" . $entity_id)); 
        die;
    }

    private function get_filter()
    {
        $where = "contract_extention.created_at IS NOT NULL AND contract_extention.contract_id =" . $this->data['entity_id'];

        $field_name = "item";
        if(isset($_GET[$field_name]) && strlen(trim($_GET[$field_name])) > 0)
        {
            $where .= " AND contract_extention." . $field_name . " LIKE '%" . trim($_GET[$field_name]) . "%'";
        }
