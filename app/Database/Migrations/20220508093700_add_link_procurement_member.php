<?php

namespace App\Database\Migrations;

use CodeIgniter\Database\Migration;

class AddLinkProcurementMember extends Migration
{
    public function up()
    {
        $this->forge->addField([
            'id'          => [
                'type'           => 'INT',
                'constraint'     => 11,
                'unsigned'       => true,
                'auto_increment' => true,
            ],
            'procurement_id'          => [
                'type'           => 'INT',
                'constraint'     => 11,
                'unsigned'       => true,
                'comment' => 'Refer procurement table primary key',
            ],
            'sub_commitee_id'          => [
                'type'           => 'INT',
                'constraint'     => 11,
                'unsigned'       => true,
                'comment' => 'Refer sub_commitee_members table primary key',
            ],
            'created_at'          => [
                'type'           => 'DATETIME',
                'null'           => true,
            ],
            'updated_at'          => [
                'type'           => 'DATETIME',
                'null'           => true,
            ],
            'deleted_at'          => [
                'type'           => 'DATETIME',
                'null'           => true,
            ],
        ]);

        $this->forge->addField('CONSTRAINT FOREIGN KEY (procurement_id) REFERENCES procurement(id)');
        $this->forge->addField('CONSTRAINT FOREIGN KEY (sub_commitee_id) REFERENCES sub_commitee_members(id)');

        $this->forge->addKey('id', true);
        $this->forge->createTable('link_procurement_member');
    }

    public function down()
    {
        $this->forge->dropTable('link_procurement_member');
    }
}