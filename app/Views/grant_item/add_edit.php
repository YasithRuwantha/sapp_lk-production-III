            ...
            <div class="page-wrapper">
                <div class="content container-fluid">
					...
					<div class="page-header">
						<div class="row">
							<div class="col">
								<h3 class="page-title">Grant Item Farmer</h3>
								<ul class="breadcrumb">
									<li class="breadcrumb-item"><a href="<?php echo base_url("/grant/list_all/"); ?>">Grant</a></li>
                                    <li class="breadcrumb-item"><a href="<?php echo base_url("/grant_disbursement/list_all/" . $parent_id); ?>">Grant Disbursement</a></li>
                                    <li class="breadcrumb-item"><a href="<?php echo base_url("/grant_item/list_all/" . $entity_id); ?>">Grant Item</a></li>
									<li class="breadcrumb-item active">Grant Item Details</li>
								</ul>
							</div>
						</div>
					</div>
						...
						<div class="col-lg-12">
							<div class="card">
								<div class="card-header">
									<h5 class="card-title">Grant Item Details</h5>
								</div>
								<div class="card-body">
                                    <?php
                                        $inc=1;
                                    ?>
									<form method="post" action="<?php echo base_url("/grant_item/add_edit/" . $entity_id . "/" . $id); ?>" enctype="multipart/form-data">
                                        <input name="csrf" value="<?php echo $csrf; ?>" type="hidden">

										<div class="row form-group">
											<label for="field-label-<?php echo $inc; ?>" class="col-form-label col-md-2">Item</label>
											<div class="col-sm-10">
                                                <?php $field_name = "project_target_item_id"; ?>
												<select class="<?php if(!(isset($project_target_item_id) && is_array($project_target_item_id) && count($project_target_item_id) > 0)){ ?>is-invalid<?php } ?> form-select<?php if($validation->hasError($field_name)){ ?> is-invalid<?php } ?> entity-type" id="field-label-<?php echo $inc++; ?>" name="<?php echo $field_name; ?>">
													<?php if(!isset($record[$field_name])){ ?>
														<option value="">-- Select --</option>
													<?php } ?>
                                                    <?php foreach($project_target_item_id as $key=>$val){ ?>
                                                        <option <?php if(isset($record[$field_name]) && $val['id'] == $record[$field_name]){ ?>selected<?php } ?> value="<?php echo $val['id']; ?>"><?php echo $val['item_description']; ?></option>
                                                    <?php } ?>
												</select>
                                                <?php if($validation->hasError($field_name)){ ?>
                                                <div class="invalid-feedback"><?php echo $validation->getError($field_name); ?></div>
                                                <?php } ?>
												<?php if(!(isset($project_target_item_id) && is_array($project_target_item_id) && count($project_target_item_id) > 0)){ ?>
												<div class="invalid-feedback">
													There is no product added to the relavent project. 
													<!--<a hrev="<?php echo base_url("/project_target/list_all/" . $entity_id); ?>" target="_blank">Create here</a>-->
												</div>
												<?php } ?>
											</div>
										</div>
