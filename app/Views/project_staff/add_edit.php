    ...
    <body>
						...
						<div class="row">
							<div class="col">
								...
								<ul class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item"><a href="<?php echo base_url("/project_staff/list_all/" . $entity_id); ?>">Project Staff</a></li>
						...
						<div class="col-lg-12">
							<div class="card">
								<div class="card-header">
									<h5 class="card-title">Project Staff Details</h5>
								</div>
								<div class="card-body">
                                    <?php
                                        $inc=1;
                                    ?>
									<form method="post" action="<?php echo base_url("/project_staff/add_edit/" . $entity_id . "/" . $id); ?>" enctype="multipart/form-data">
                                        <input name="csrf" value="<?php echo $csrf; ?>" type="hidden">

										<div class="row form-group">
											<label for="field-label-<?php echo $inc; ?>" class="col-form-label col-md-2">Role</label>
											<div class="col-sm-10">
                                                <?php $field_name = "role"; ?>
										...
										<div class="row form-group">
											<label for="field-label-<?php echo $inc; ?>" class="col-form-label col-md-2">Staff</label>
											<div class="col-sm-10">
                                                <?php $field_name = "user_id"; ?>
												<select class="staff-select form-select<?php if($validation->hasError($field_name)){ ?> is-invalid<?php } ?>" id="field-label-<?php echo $inc++; ?>" name="<?php echo $field_name; ?>">
													<?php if(!isset($record[$field_name])){ ?>
														<option value="">-- Select --</option>
													<?php } ?>
                                                    <?php foreach($user_list as $key=>$val){ ?>
                                                        <option class="entity" <?php if(isset($record[$field_name]) && $val['id'] == $record[$field_name]){ ?>selected<?php } ?> value="<?php echo $val['id']; ?>"><?php echo $val['fname']; ?> <?php echo $val['lname']; ?></option>
													<?php } ?>
												</select>
                                                <?php if($validation->hasError($field_name)){ ?>
                                                <div class="invalid-feedback"><?php echo $validation->getError($field_name); ?></div>
		...
		$( function() {
			...
			$('.entity-type').on('change', function () {
            	$.post("<?php echo base_url("project_staff/get_staff_list"); ?>" + "/" + $(this).find(":selected").val(), {entity: "<?php echo "15"; ?>"})
                	.done(function (resData) {
                    	$('.staff-select').html(resData);
                });
        	});
