            ...
            <div class="page-wrapper">
                <div class="content container-fluid">
					...
					<div class="page-header">
						<div class="row">
							<div class="col">
								<h3 class="page-title">Staff Contract Management</h3>
								<ul class="breadcrumb">
									<li class="breadcrumb-item"><a href="<?php echo base_url("/staff_contract_mgt/list_all/"); ?>">Staff Contract Management</a></li>
									<li class="breadcrumb-item active">Staff Contract Management Details</li>
								</ul>
							</div>
						</div>
					</div>
						...
						<div class="col-lg-12">
							<div class="card">
								<div class="card-header">
									<h5 class="card-title">Staff Contract Management Details</h5>
								</div>
								<div class="card-body">
                                    <?php
                                        $inc=1;
                                    ?>
									<form method="post" action="<?php echo base_url("/staff_contract_mgt/add_edit/" . $id); ?>" enctype="multipart/form-data">
                                        <input name="csrf" value="<?php echo $csrf; ?>" type="hidden">

                                        <div class="row form-group">
											<?php $field_name = "user_id"; ?>
											<label for="field-label-<?php echo $inc; ?>" class="col-form-label col-md-2"><?php echo ucfirst(str_replace("_"," ","user")); ?></label>
											<div class="col-sm-10">                                                
												<select class="<?php echo $field_name; ?> search-select form-select<?php if($validation->hasError($field_name)){ ?> is-invalid<?php } ?>" id="field-label-<?php echo $inc++; ?>" name="<?php echo $field_name; ?>">
                                        ...
                                        <div class="form-group row">
											...
											<div class="col-sm-10">                                                
												<input type="text" placeholder="" class="<?php echo $field_name; ?> form-control<?php if($validation->hasError($field_name)){ ?> is-invalid<?php } ?>" id="field-label-<?php echo $field_name; ?>" name="<?php echo $field_name; ?>" value="<?php if(isset($record[$field_name])){ echo $record[$field_name]; } ?>">
                                                <?php if($validation->hasError($field_name)){ ?>
                                                <div class="invalid-feedback"><?php echo $validation->getError($field_name); ?></div>
                                                <?php } ?>
                                            </div>
                                        ...
                                        <div class="row form-group">
											...
											<div class="col-sm-10">                                                
                                                ...
                                                <div class="invalid-feedback"><?php echo $validation->getError($field_name); ?></div>
                                                <?php } ?>
												<div class="ajax-<?php echo $field_name; ?>"></div>
											</div>
										</div>
		...
		$( function() {
			...
			$( ".<?php echo $field_name; ?>" ).datepicker({ 
				...
				onSelect: function(date){            
        			var date1 = $('.contract_effective_date').datepicker('getDate');           
        			var date = new Date( Date.parse( date1 ) ); 
        			date.setDate( date.getDate() + 0 );        
        			var newDate = date.toDateString(); 
        			newDate = new Date( Date.parse( newDate ) );                      
        			$('.contract_expiary_date').datepicker("option","minDate",newDate);            
    			}
			});
			...
			$( ".<?php echo $field_name; ?>" ).datepicker({ 
				...
				changeYear: true,
				onSelect: function(date){            
        			var date2 = $('.contract_expiary_date').datepicker('getDate');            
        			var date = new Date( Date.parse( date2 ) ); 
        			date.setDate( date.getDate() + 0);        
        			var newDate = date.toDateString(); 
        			newDate = new Date( Date.parse( newDate ) );                      
        			$('.contract_effective_date').datepicker("option","maxDate",newDate);            
    			}
			});
			$( ".<?php echo $field_name; ?>" ).datepicker( "option", "dateFormat", "yy-mm-dd" );
			setTimeout($( ".<?php echo $field_name; ?>" ).val( "<?php if(isset($record[$field_name])){ echo $record[$field_name]; } ?>" ), 200);
			...
			$( ".user_id" ).change(function() {
				...
				$.get( "<?php echo base_url("/staff_contract_mgt/ajax_is_active_staff"); ?>/" + selected_user, function( data ) {
					$( ".ajax-contract_status" ).html( data );
				});
			});			
		});
		</script>
