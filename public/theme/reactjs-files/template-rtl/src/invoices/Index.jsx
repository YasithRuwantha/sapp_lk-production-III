					...
					<div className="page-header">
						<div className="row align-items-center">
							...
							<div className="col-auto">
								<Link to="/add-invoice" className="btn btn-primary m-1">
									<i className="fas fa-plus"></i>
								</Link>
								<a className="btn btn-primary filter-btn" href="/invoices#" id="filter_search">
							...
							<div className="card card-table"> 
								<div className="card-body">
									<div className="table-responsive">
										<table className="table table-stripped table-hover datatable" id="invoicestabe">
                                            ...
                                            {invoices.map((item, index) => {
                                                    let status;
                                                    if(item.status === 'Paid') {
                                                        status = <span className="badge badge-pill bg-success-light">{item.status}</span>
                                                    } else if(item.status === 'Overdue') {
                                                        status = <span className="badge badge-pill bg-danger-light">{item.status}</span>
                                                    }  else if(item.status === 'Sent') {
                                                        status = <span className="badge badge-pill bg-info-light">{item.status}</span>
                                                    } else if(item.status === 'Partially Paid') {
                                                        status = <span className="badge badge-pill bg-warning-light">{item.status}</span>
                                                    } 
                                                    return (
												<tr key={index}>
													<td><Link to="/view-invoice">{item.invoice_number}</Link></td>
													<td>
														<h2 className="table-avatar">
															<Link to="/profile"><img className="avatar avatar-sm me-2 avatar-img rounded-circle" src={item.customer_image}alt="User Image" /> {item.customer_name}</Link>
														</h2>
													</td>
